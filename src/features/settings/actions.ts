...
export async function updateSettingsAction(formData:FormData){const data=schema.parse({...Object.fromEntries(formData),defaultReplyTo:formData.get("defaultReplyTo")||undefined});const {workspace,userId}=await currentWorkspace({admin:true});await requireDb().transaction(async(tx)=>{await tx.update(workspaceSettings).set({...data,updatedAt:new Date()}).where(eq(workspaceSettings.workspaceId,workspace.id));await tx.insert(auditEvents).values({action:"workspace.settings_updated",actorUserId:userId,entityId:workspace.id,entityType:"workspace",workspaceId:workspace.id})});revalidatePath("/dashboard/parametres")}
