...
export default async function DocsPage() {
  ...
  return <PageShell eyebrow={copy.eyebrow} title={copy.title} intro={copy.intro}><div className="grid gap-8 lg:grid-cols-[1fr_1.3fr]"><div><h2 className="text-2xl font-semibold">{copy.heading}</h2><p className="mt-3 leading-7 text-muted-foreground">{copy.text}</p><div className="mt-6 rounded-md border bg-card p-5 text-sm"><p><strong>POST</strong> /v1/emails</p><p className="mt-2 text-muted-foreground">Authorization: Bearer ym_live_…</p><p className="text-muted-foreground">Idempotency-Key: order-1234</p></div><p className="mt-5 text-sm"><a className="text-primary underline" href="/openapi.json">{copy.download}</a></p></div><pre className="overflow-x-auto rounded-md bg-card p-6 text-sm leading-6 text-zinc-200"><code>{example}</code></pre></div></PageShell>;
}
