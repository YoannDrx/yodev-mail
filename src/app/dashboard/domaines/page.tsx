    ...
    <DashboardPage title={copy.title} description={copy.description} action={<form action={addDomainAction} className="flex gap-2"><Input aria-label={copy.domain} name="domain" placeholder="notifications.client.com" required /><Button type="submit"><Plus />{copy.add}</Button></form>}>
      <div className="grid gap-5">
        {rows.map(({ domain, binding }) => (
          <article className="rounded-md border bg-card " key={domain.id}>
            ...
            <div className="grid gap-4 p-6 sm:grid-cols-3">{[["DKIM", binding?.dkimStatus], ["Return-Path", binding?.returnPathStatus], ["DMARC", binding?.dmarcStatus]].map(([label, status]) => <div className="rounded-md border p-4" key={label}><p className="text-xs text-muted-foreground">{label}</p><p className={`mt-2 flex items-center gap-2 font-medium ${status === "verified" ? "text-[var(--y-success)]" : "text-[var(--y-warning)]"}`}>{status === "verified" ? <CheckCircle2 className="size-4" /> : <ShieldAlert className="size-4" />}{status ? statusLabel(locale,status) : copy.pending}</p></div>)}</div>
            <div className="border-t p-6"><h3 className="font-medium">{copy.records}</h3><div className="mt-3 grid gap-2">{(binding?.dnsRecords ?? []).map((record, index) => <div className="grid gap-1 rounded-lg bg-muted p-3 font-mono text-xs sm:grid-cols-[70px_1fr_1.5fr]" key={`${record.name}-${index}`}><strong>{record.type}</strong><span className="break-all">{record.name}</span><span className="break-all text-muted-foreground">{record.value}</span></div>)}</div>{binding?.lastCheckError && <p className="mt-3 text-sm text-destructive">{copy.dnsPending}</p>}</div>
          </article>
        ))}
        {!rows.length && <div className="rounded-md border border-dashed p-12 text-center text-sm text-muted-foreground">{copy.empty}</div>}
      </div>
    </DashboardPage>
  );
}
