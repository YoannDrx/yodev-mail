...
export default async function PricingPage() {
  ...
  return <PageShell eyebrow={copy.eyebrow} title={copy.title} intro={copy.intro}><div className="mx-auto max-w-xl"><article className="relative rounded-md border border-primary bg-card p-8  shadow-primary/10"><Badge className="absolute right-6 top-6">{copy.badge}</Badge><h2 className="text-2xl font-semibold">{copy.platform}</h2><p className="mt-4 text-4xl font-semibold">{copy.price}<span className="text-sm font-normal text-muted-foreground"> /{copy.month}</span></p><p className="mt-3 text-lg font-medium">{copy.usage}</p><p className="mt-1 text-sm text-muted-foreground">{copy.example}</p><ul className="mt-7 grid gap-3 text-sm">{copy.features.map((item)=><li className="flex gap-2" key={item}><Check className="size-4 text-[var(--y-success)]"/>{item}</li>)}</ul><Button asChild className="mt-7 w-full"><Link href={localizedPath(locale, "/inscription")}>{copy.apply}</Link></Button></article></div><p className="mt-8 text-center text-sm text-muted-foreground">{copy.tax}</p></PageShell>;
}
